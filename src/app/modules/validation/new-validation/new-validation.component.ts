...
export class NewValidationComponent implements OnInit {
  ...
  constructor(private formBuilder:FormBuilder, private validationservice:ValidationsService, private roleservices:RolesService, private router:Router, private infosPath:ActivatedRoute, private donneeEchancgesService: DonneesEchangeService) {
  ...
  ngOnInit(): void {
    this.donneeEchancgesService.getTypeValidation().subscribe((x) => (this.typeValidations = x.type))
